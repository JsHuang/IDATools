...
class StringsManager():
    ...
    def build_strings_tree(self):
        ...
        sorted(self.list_keys)

    def func_name_to_addr(self, func_name):
        for func_addr in Functions():
            curr_func_name = idc.get_func_name(func_addr)
    ...
    def get_current_function_name(self):
        return self.get_func_name(list(self.list_keys)[self.current_function_index])

    def get_current_function_addr(self):
        return list(self.list_keys)[self.current_function_index]
...
class FunctionRenamer(ida_idaapi.plugin_t):
    ...
    def run(self, arg):
        ...
        if FunctionRenamer.SHOW_MODEL:
            ...
            # Set flags
            rf.modal = False
            rf.openform_flags = ida_kernwin.PluginForm.WOPN_TAB
            # Compile (in order to populate the controls)
            rfc, _ = rf.Compile()
